...
export class ServiceService {
  ...
  async findAll(): Promise<Service[]> {
    return this.serviceModel.find().populate('institutions').populate('fields').exec();
  }

  async findOne(id: string): Promise<Service> {
    const service = await this.serviceModel.findById(id).populate('institutions').populate('fields').exec();
    if (!service) {
  ...
  async removeInstitutionFromService(serviceId: string, institutionId: string): Promise<Service> {
    ...
    if (institutionIndex === -1) {
        throw new NotFoundException(`Institution with ID ${institutionId} not found in service`);
    }
  ...
  async addInstitutionToService(serviceId: string, institutionId: string): Promise<Service> {
    ...
    if (service.institutions.includes(institution.id)) {
      throw new ConflictException('Institution already associated with this service');
    }
