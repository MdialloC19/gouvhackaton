...
export class FonctionnaireService {
  constructor(
    @InjectModel(Fonctionnaire.name) private readonly fonctionnaireModel: Model<Fonctionnaire>,
  ...
  async create(createFonctionnaireDto: CreateFonctionnaireDto): Promise<Fonctionnaire> {
    ...
    if (!institution) {
      throw new NotFoundException(`Institution with name ${institutionName} not found`);
    }

    const fonctionnaire = new this.fonctionnaireModel({ ...rest, institution: institution._id });
  ...
  async findOne(id: string): Promise<Fonctionnaire> {
    const fonctionnaire = await this.fonctionnaireModel.findById(id).populate('institution').exec();
    if (!fonctionnaire) {
      throw new NotFoundException(`Fonctionnaire with ID "${id}" not found`);
    }
    return fonctionnaire;
  }

  async update(id: string, updateFonctionnaireDto: UpdateFonctionnaireDto): Promise<Fonctionnaire> {
    const updatedFonctionnaire = await this.fonctionnaireModel.findByIdAndUpdate(id, updateFonctionnaireDto, {
      new: true,
    }).exec();
  ...
  async remove(id: string): Promise<Fonctionnaire> {
    const deletedFonctionnaire = await this.fonctionnaireModel.findByIdAndDelete(id).exec();
